...
exports.LOOP_CONFIG = exports.Phase4Loop = void 0;
const dotenv_1 = require("dotenv");
const traderJoeEvaluator_js_1 = require("../engine/traderJoeEvaluator.js");
const phase4_js_1 = require("../metrics/phase4.js");
const env_js_1 = require("../config/env.js");
const logger_js_1 = require("../engine/logger.js");
const telegram_js_1 = require("../notifications/telegram.js");
...
exports.LOOP_CONFIG = LOOP_CONFIG;
/**
...
class Phase4Loop {
    ...
    constructor() {
        this.evaluator = (0, traderJoeEvaluator_js_1.createTraderJoeEvaluator)();
     ...
     */
    async start() {
        logger_js_1.logger.info('🚀 Starting Phase 4 continuous dry-run evaluation loop');
        try {
            ...
            logger_js_1.logger.error('❌ Phase 4 loop startup failed:', error);
            this.logStructured({
                type: 'loop_error',
                timestamp: new Date().toISOString(),
     ...
     */
    stop() {
        logger_js_1.logger.info('🛑 Stopping Phase 4 evaluation loop...');
        this.running = false;
        ...
        while (this.running && !this.gracefulShutdown) {
            ...
            try {
                ...
                if (LOOP_CONFIG.maxIterations > 0 && this.iteration > LOOP_CONFIG.maxIterations) {
                    logger_js_1.logger.info(`📊 Reached maximum iterations (${LOOP_CONFIG.maxIterations}), stopping...`);
                    break;
                }
                ...
                logger_js_1.logger.error(`❌ Error in iteration ${this.iteration}:`, error);
                this.logStructured({
    ...
    async performEvaluations() {
        for (const [tokenInSymbol, tokenOutSymbol] of LOOP_CONFIG.testPairs) {
            for (const amountUsd of LOOP_CONFIG.testAmounts) {
                ...
                try {
                    const startTime = Date.now();
                    const evaluation = await (0, traderJoeEvaluator_js_1.quickEvaluate)(tokenInSymbol, tokenOutSymbol, amountUsd);
                    ...
                    logger_js_1.logger.info(`[${this.iteration}] ${result.pair} ($${amountUsd}): ${status} - Profit: $${evaluation.profitUsd.toFixed(2)}, Score: ${evaluation.executionScore.toFixed(1)}/100`);
                    // Check if we should send notifications based on evaluation status
                    await this.handleEvaluationNotifications(result);
                }
                catch (error) {
                    logger_js_1.logger.error(`❌ Evaluation failed for ${tokenInSymbol}/${tokenOutSymbol}:`, error);
                    this.logStructured({
     ...
     */
    async logIterationMetrics(iterationTimeMs) {
        const metrics = phase4_js_1.phase4Metrics.getMetrics();
        const routerMetrics = Array.from(metrics.routerPerformance.values());
        const avgResponseTime = routerMetrics.length > 0
            ? routerMetrics.reduce((sum, r) => sum + r.averageResponseTime, 0) / routerMetrics.length
     ...
     */
    async shutdown() {
        logger_js_1.logger.info('🔄 Performing graceful shutdown...');
        // Log final metrics
        this.logStructured({
            type: 'loop_stopped',
            timestamp: new Date().toISOString(),
        ...
        logger_js_1.logger.info('✅ Phase 4 evaluation loop stopped gracefully');
    }
    /**
    ...
    setupGracefulShutdown() {
        const shutdown = () => {
            if (!this.gracefulShutdown) {
                logger_js_1.logger.info('🔄 Received shutdown signal, initiating graceful shutdown...');
                this.gracefulShutdown = true;
                this.stop();
                // Force exit after timeout
                setTimeout(() => {
                    logger_js_1.logger.warn('⚠️ Graceful shutdown timeout, forcing exit');
                    process.exit(1);
    ...
    async sendStartupNotification() {
        try {
            const alert = (0, telegram_js_1.createSystemStatusAlert)('Phase 4 evaluation loop started successfully', {
            ...
            await (0, telegram_js_1.sendTelegramAlert)(alert);
        }
        catch (error) {
            logger_js_1.logger.warn('Failed to send startup notification:', error);
        }
    }
    /**
     * Handle evaluation-based notifications
     */
    async handleEvaluationNotifications(result) {
        try {
            ...
            if (result.evaluation.profitable &&
                ...
                if (now - lastNotification >= LOOP_CONFIG.notifications.cooldownMs) {
                    const alert = (0, telegram_js_1.createProfitableOpportunityAlert)(result.pair, result.amountUsd, result.evaluation.profitUsd, result.evaluation.netUsd, result.evaluation.executionScore);
                    const sent = await (0, telegram_js_1.sendTelegramAlert)(alert);
                    if (sent) {
            ...
            logger_js_1.logger.warn('Failed to handle evaluation notifications:', error);
        }
    }
    /**
    ...
    async handleErrorNotification(error, context) {
        try {
            ...
            if (now - lastNotification >= LOOP_CONFIG.notifications.cooldownMs * 2) { // Longer cooldown for errors
                ...
                const alert = (0, telegram_js_1.createExecutionErrorAlert)(errorMessage, context);
                const sent = await (0, telegram_js_1.sendTelegramAlert)(alert);
                if (sent) {
            ...
            logger_js_1.logger.warn('Failed to send error notification:', notificationError);
        }
    }
    /**
    ...
    async sendSummaryNotification() {
        try {
            ...
            const alert = (0, telegram_js_1.createEvaluationSummaryAlert)(metrics.totalEvaluations, recentOpportunities.length, averageProfit, timespan);
            await (0, telegram_js_1.sendTelegramAlert)(alert);
        }
        catch (error) {
            logger_js_1.logger.warn('Failed to send summary notification:', error);
        }
    }
}
exports.Phase4Loop = Phase4Loop;
/**
...
async function main() {
    try {
        ...
        logger_js_1.logger.error('❌ Phase 4 loop failed to start:', error);
        process.exit(1);
    }
}
...
//# sourceMappingURL=runPhase4Loop.js.map