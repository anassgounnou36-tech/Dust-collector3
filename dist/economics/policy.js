...
exports.Policy = exports.PROTOCOL_POLICIES = void 0;
exports.validatePolicyThresholds = validatePolicyThresholds;
exports.adjustPolicyForMarketConditions = adjustPolicyForMarketConditions;
...
exports.Policy = env_js_1.env.devLowerThresholds
    ? { ...BASE_POLICY, ...DEV_OVERRIDES }
    : BASE_POLICY;
// Enhanced policy validation
function validatePolicyThresholds() {
    const warnings = [];
    if (exports.Policy.MIN_BUNDLE_NET_USD > exports.Policy.MIN_BUNDLE_GROSS_USD) {
        warnings.push('MIN_BUNDLE_NET_USD cannot be greater than MIN_BUNDLE_GROSS_USD');
    }
    if (exports.Policy.MIN_BUNDLE_SIZE > exports.Policy.MAX_BUNDLE_SIZE) {
        warnings.push('MIN_BUNDLE_SIZE cannot be greater than MAX_BUNDLE_SIZE');
    }
    if (exports.Policy.PRICE_IMPACT_WARNING_PCT > exports.Policy.PRICE_IMPACT_MAX_PCT) {
        warnings.push('PRICE_IMPACT_WARNING_PCT should be less than PRICE_IMPACT_MAX_PCT');
    }
    if (exports.Policy.MIN_EXECUTION_SCORE < 0 || exports.Policy.MIN_EXECUTION_SCORE > 100) {
        warnings.push('MIN_EXECUTION_SCORE should be between 0 and 100');
    }
    return warnings;
}
// Dynamic policy adjustments based on market conditions
function adjustPolicyForMarketConditions(gasPrice, volatility) {
    const adjustments = {};
    // Adjust gas-sensitive thresholds during high gas periods
    if (gasPrice > 50) { // High gas (>50 gwei)
        adjustments['MIN_BUNDLE_NET_USD'] = exports.Policy.MIN_BUNDLE_NET_USD * 1.5;
        adjustments['MIN_PROFIT_USD'] = exports.Policy.MIN_PROFIT_USD * 2;
    }
    // Adjust slippage during high volatility
    if (volatility > 0.05) { // High volatility (>5%)
        adjustments['MAX_SLIPPAGE_PCT'] = Math.max(exports.Policy.MAX_SLIPPAGE_PCT * 0.8, 2.0);
        adjustments['PRICE_IMPACT_MAX_PCT'] = Math.max(exports.Policy.PRICE_IMPACT_MAX_PCT * 0.8, 3.0);
    }
    return adjustments;
}
