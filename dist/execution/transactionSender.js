...
exports.TransactionSender = exports.ExecutionMode = void 0;
exports.createTransactionSender = createTransactionSender;
...
class TransactionSender {
    ...
    constructor(config) {
        ...
        logger_js_1.logger.info(`🔧 TransactionSender initialized - DRY_RUN_ONLY: ${this.config.dryRunOnly}`);
        if (this.config.dryRunOnly) {
            logger_js_1.logger.info('🧪 Running in DRY-RUN mode - no real transactions will be sent');
        }
        else {
            logger_js_1.logger.warn('⚠️ LIVE MODE ENABLED - real transactions will be sent!');
        }
    }
    /**
    ...
    async executeTransaction(params) {
        ...
        try {
            ...
            logger_js_1.logger.error('❌ Transaction execution failed:', error);
            this.logStructured({
    ...
    async executeBatch(transactions) {
        logger_js_1.logger.info(`📦 Executing batch of ${transactions.length} transactions`);
        const results = [];
        for (let i = 0; i < transactions.length; i++) {
            const tx = transactions[i];
            logger_js_1.logger.info(`🔄 Processing transaction ${i + 1}/${transactions.length}`);
            try {
                ...
                logger_js_1.logger.error(`❌ Batch transaction ${i + 1} failed:`, error);
                results.push({
                    success: false,
        ...
        logger_js_1.logger.info(`📊 Batch execution complete: ${successful}/${transactions.length} successful`);
        this.logStructured({
    ...
    async performSafetyChecks(params) {
        ...
        if (params.quote.priceImpact > 0.05) { // 5%
            logger_js_1.logger.warn(`⚠️ High price impact: ${(params.quote.priceImpact * 100).toFixed(2)}%`);
        }
        // Additional safety checks would go here
        logger_js_1.logger.debug('✅ Safety checks passed');
    }
    /**
     * Simulate transaction execution in dry-run mode
     */
    async simulateTransaction(params, timestamp) {
        logger_js_1.logger.info('🧪 Simulating transaction execution...');
        ...
        logger_js_1.logger.info(`✅ Transaction simulation successful - Mock TX: ${mockTxHash}`);
        this.logStructured({
     ...
     */
    async executeLiveTransaction(params, timestamp) {
        logger_js_1.logger.warn('🚨 LIVE TRANSACTION EXECUTION - This is a placeholder implementation');
    ...
    logIntendedAction(params, mode) {
        ...
        logger_js_1.logger.info(`💼 Intended Action: ${action.details.inputToken} → ${action.details.outputToken} (${mode.toUpperCase()})`);
    ...
    updateConfig(newConfig) {
        ...
        logger_js_1.logger.info('🔧 TransactionSender configuration updated');
    }
    /**
 ...
 */
function createTransactionSender(config) {
 ...
 * Default export
 */
exports.default = TransactionSender;
